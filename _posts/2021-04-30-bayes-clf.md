...
* [Building the Model](#model)
    - Brief run down on the model being built and code necessary to build a classifier to predict targets of Iris flower species. 

* [Testing the Model](#test)
    - Evaluating model on test dataset, and comparing against maximum likelihood version. 
...
(150, 4) (3,)
```
Now we know there is total of 150 samples, 4 features, and 3 targets. As all good EDA should do, we should do visualization to see whats going on with the data, and with only 4 features we can easily do pairplot to see how each features match up to others.
```python
iris = X.copy()
iris['target'] = y.map({0:'setosa', 1:'versicolor', 2:'virginica'})
pplot = sns.pairplot(iris, hue="target")
```
![irispairplot](https://raw.githubusercontent.com/chuanfuyap/mini-ds-projects/main/bayes-clf/irispair.png)
...
To build our model, I'll be modelling the dataset with Multivariate Gaussian Distribution for the features
...
```
![model-graph](https://github.com/chuanfuyap/mini-ds-projects/blob/main/bayes-clf/bayesclf.png?raw=true)
We can see we built the model correctly with the `for loop`, resulting in 3 models, 1 for each class.

Now we will actually fit the model by running the Bayesian inference engine
```python
...
pm.traceplot(trace)
```
The column on the left is the distribution for the fitted parameters, and the right column is the trace plot, which is the final distribution random sample values (default distribution sample size is 2000). Sampling means drawing random samples from the distribution from our model. Much like scipy's `norm.rvs(size=1000)` which is drawing 1000 random samples from a normal distribution. 
![traceplot](https://github.com/chuanfuyap/mini-ds-projects/blob/main/bayes-clf/traceplot.png?raw=true)
...
This is done by computing the probability density using fitted parameters for each class, and selecting the class that has the highest probabilty density of the classes for the value of interest. 
...
Now that we have fitted model and have a way to make predictions with it, we should evaluate it. 
```python
...
Another approach to building models like this is to do it with maximum likelihood approach which is taking the mean and covariance without caring bout its dispersion and use that as model parameters. Scikit-learn has a module for this, we can implement that and see how it performs.
```python
...
My previous code block for prediction gives a hard prediction without the probabilities, which would not really do in the probabilistic modelling scene! Below is a code block for prediction probabilties which would allow us to score our model with area under roc.
...
If that didn't explain any of it to you clearly, the best explanation I have ever read is by [Jake VanderPlas](http://jakevdp.github.io/pages/about.html) from his [series of post](http://jakevdp.github.io/blog/2014/03/11/frequentism-and-bayesianism-a-practical-intro/) regarding this, and the best line regarding frequentism vs bayesian is the following:
